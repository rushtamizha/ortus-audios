...
export default async function BlogPostPage({ params }) {
  ...
  const heroImage = (post.featuredImage && post.featuredImage.trim() !== "") 
    ? post.featuredImage 
  ...
  return (
    <article className="min-h-screen bg-[#020205] text-white">
      ...
      <div className="relative h-[60vh] w-full bg-[#08080c]">
        {heroImage && (
          <img src={heroImage} className="absolute inset-0 w-full h-full object-cover opacity-30" alt={post.title} />
        )}
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 sm:p-20 z-10">
          <div className="max-w-6xl mx-auto">
            <Link href="/blog" className="group flex items-center gap-2 text-[#8B0000] uppercase text-[10px] font-black tracking-widest mb-8">
              <ChevronLeft size={14} className="group-hover:-translate-x-1 transition-transform"/> Back to Ledger
            </Link>
            <h1 className="text-5xl md:text-7xl font-black leading-[0.9] tracking-tighter uppercase italic">{post.title}</h1>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-20 grid grid-cols-1 lg:grid-cols-4 gap-16">
        ...
        <div className="lg:col-span-3">
          <div className="flex items-center gap-8 mb-12 text-white/20 uppercase text-[10px] font-black tracking-widest">
            <span className="flex items-center gap-2"><User size={12} className="text-[#8B0000]"/> {post.author || "Staff"}</span>
            <span className="flex items-center gap-2"><Calendar size={12} className="text-[#8B0000]"/> {new Date(post.createdAt).toLocaleDateString()}</span>
          </div>
